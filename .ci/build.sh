...
for ly in ly/**/*.ly; do
	...
	ffmpeg -i dist/"$path"/"$name".wav -metadata title="$(grep '^\s\+title\s*=\s*.*$' "$ly" | sed -e 's/^[^"]*"\(.*\)"\s*$/\1/')" -metadata comment="$(grep '^\s\+instrument\s*=\s*.*$' "$ly" | sed -e 's/^[^"]*"\(.*\)"\s*$/\1/')" -metadata composer="$(grep '^\s\+composer\s*=\s*.*$' "$ly" | sed -e 's/^[^"]*"\(.*\)"\s*$/\1/')" -metadata artist="$(grep '^\s\+arranger\s*=\s*.*$' "$ly" | sed -e 's/^[^"]*"\(.*\)"\s*$/\1/')" dist/"$path"/"$name".m4a
done
